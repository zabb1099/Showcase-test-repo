...
use App\Http\Requests\ITPortal\DevSupportJobs\DuplicateFile\ClientFilesIVARequest;
use App\Jobs\ITPortal\ITSupport\DuplicateFile\DeleteDuplicateIVADebtsolvFiles;
use App\Jobs\ITPortal\ITSupport\DuplicateFile\UpdateDebtsolvIVAClientId;
use Illuminate\Support\Facades\DB;


class DuplicateDebtsolvFileIVAController extends Controller
{
    public function deleteDuplicateDebtsolvFilesIVA(ClientFilesIVARequest $clientFileIds, AuditReasonRequest $reasonRequest)
    {
         ...
         if (count(array_unique($this->checkClientDetailsAreTheSame($clientIds))) === 1) {
             ...
             if (count(array_unique($this->checkLeadReferencesMatch($clientIds))) === 1) {
                 ...
                 if ($this->checkLeadsDatabaseHasCorrectDebtsolvClientId($clientIds) != $clientIdToKeep) {
                     ...
                     UpdateDebtsolvIVAClientId::dispatchSync(
                         $clientIdToKeep,
                         $leadReference[0],
                 ...
                 if ($this->checkFilesToBeDeletedAreNotTheLatestDebtsolvFileCreated() != $clientIdsToDelete) {
                     return DeleteDuplicateIVADebtsolvFiles::dispatchSync(
                         $clientIdsToDelete,
                         $reasonRequest->reason
                     );
                 }
    ...
    {
        $getClients = DB::connection('debtsolv_iva')
    ...
    {
        $getLeadReferences = DB::connection('debtsolv_iva')
    ...
    {
        $checkForMoreLeadReferences = DB::connection('debtsolv_iva')
            ->table('Client_LeadData')
            ->select('Client_ID')
    ...
    {
        $lastFileCreated = DB::connection('debtsolv_iva')
            ->table('Client_Contact')
